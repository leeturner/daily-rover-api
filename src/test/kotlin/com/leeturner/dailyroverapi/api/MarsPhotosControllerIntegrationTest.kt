...
internal class MarsPhotosControllerIntegrationTest(
  ...
  @MockkBean(relaxed = true) private lateinit var restTemplate: RestTemplate

  @Test
  internal fun `the default endpoint calls the nasa api for yesterdays date`() {

    val expectedEarthDate = LocalDate.now().minusDays(1)
    this.mockMvc.perform(get("/v1/photos/")).andExpect(status().isOk).andDo(print())
  ...
  internal fun `the get photos endpoint returns the correct payload`() {
    ...
    every {
      restTemplate.getForEntity(
          ...
          NasaPhotoResponse::class.java)
    } answers { ResponseEntity.ok(nasaPhotoResponse) }

    every {
      restTemplate.getForEntity(
          "https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?api_key=DEMO_KEY&earth_date=$earthDate",
          NasaPhotoResponse::class.java)
    } answers { ResponseEntity.ok(NasaPhotoResponse(photos = listOf())) }
    ...
    this.mockMvc
        ...
        .andExpect(
            jsonPath(
                "\$.photos[0].earth_date", `is`(nasaPhotoResponse.photos[0].earthDate.toString())))
        ...
        .andExpect(
            jsonPath("\$.photos[0].camera.name", `is`(nasaPhotoResponse.photos[0].camera.name)))
        .andExpect(
            jsonPath(
                "\$.photos[0].camera.rover_id", `is`(nasaPhotoResponse.photos[0].camera.roverId)))
        .andExpect(
            jsonPath(
                "\$.photos[0].camera.full_name", `is`(nasaPhotoResponse.photos[0].camera.fullName)))
        .andExpect(jsonPath("\$.photos[0].rover.id", `is`(nasaPhotoResponse.photos[0].rover.id)))
        .andExpect(
            jsonPath("\$.photos[0].rover.name", `is`(nasaPhotoResponse.photos[0].rover.name)))
        .andExpect(
            jsonPath(
                "\$.photos[0].rover.landing_date",
                `is`(nasaPhotoResponse.photos[0].rover.landingDate.toString())))
        .andExpect(
            jsonPath(
                "\$.photos[0].rover.launch_date",
                `is`(nasaPhotoResponse.photos[0].rover.launchDate.toString())))
        .andExpect(
            jsonPath("\$.photos[0].rover.status", `is`(nasaPhotoResponse.photos[0].rover.status)))
        .andDo(print())
  ...
  internal fun `get photos for a date where no rovers were on mars does not call the nasa api`() {
    ...
    this.mockMvc.perform(get("/v1/photos/$earthDate")).andExpect(status().isOk).andDo(print())
  ...
  internal fun `get photos only calls the nasa api for the rovers that are active on mars for the specified date`() {
    // only opportunity and curiosity should have been active on this date.  It was after spirit
    // completed
