...
class MarsPhotosService(
  ...
  fun getPhotosByEarthDateAndMarRover(earthDate: LocalDate, rover: Rover): List<Photo> {
    ...
    if (earthDate < rover.landingDate) {
      logger.info {
        "Mars Rover (${rover.name}) was not on Mars for the specified date - $earthDate. No API call needs to be made"
      }
      return listOf()
    }
    ...
    rover.maxDate?.let {
      if (earthDate > rover.maxDate && rover.status == RoverStatus.COMPETE.status) {
        logger.info {
          "Mars Rover (${rover.name}) had ended its mission on Mars for the specified date - $earthDate. No API call needs to be made"
        }
        return listOf()
      }
    }
